...
st.markdown("""
    ...
    /* Green background, max text size, centered button behavior */
    .stButton>button {
    ...
    /* Chatbot specific smaller buttons */
    .chat-btn>button { font-size: 14px !important; padding: 4px 10px !important; background-color: #1e3a5f !important; border: 1px solid #3a7bd5 !important; }
    .chat-btn>button:hover { background-color: #3a7bd5 !important; }
    
    /* Massive, highly visible tab headers at the top */
    ...
    .flow-container { display: flex; flex-wrap: wrap; justify-content: center; align-items: flex-start; gap: 10px; background-color: #132235; padding: 30px 10px; border-radius: 16px; border: 1px solid #1e3a5f; margin-bottom: 30px; }
    ...
    /* Chat bubbles */
    .user-msg { background-color: #1e3a5f; padding: 10px; border-radius: 8px; margin-bottom: 10px; text-align: right; border-right: 4px solid #4caf50;}
    .ai-msg { background-color: #132235; padding: 10px; border-radius: 8px; margin-bottom: 10px; text-align: left; border-left: 4px solid #3a7bd5;}
    </style>
...
# --- LOCAL AI CONTEXT ENGINE (Simulated RAG) ---
def generate_ai_response(query):
    q = query.lower()
    if any(word in q for word in ["medallion", "architecture"]):
        return "The Medallion Architecture is a data design pattern that logically organizes data in a lakehouse approach. We use 3 layers: Bronze (raw ingestion), Silver (cleansed & typed), and Gold (business-ready warehouse)."
    elif "bronze" in q:
        return "The Bronze Layer is our fault-tolerant ingestion zone. It fetches raw JSON from CoinCap and YFinance and stores it unaltered to ensure absolute data lineage and recovery backups."
    elif "silver" in q:
        return "The Silver Layer handles data cleansing. We extract the messy Bronze JSON, cast text to numeric floats, drop invalid records, and compress the output into highly optimized PyArrow Parquet formats."
    elif "gold" in q:
        return "The Gold Layer is our relational Data Warehouse. Clean Silver data is appended into an SQLite database with an `ingested_at` timestamp, allowing us to perform historical time-series tracking."
    elif any(word in q for word in ["ml", "machine learning", "predict", "ai"]):
        return "We utilize a local Scikit-Learn Linear Regression model. It trains dynamically on the historical 'priceUsd' data points in the Gold database to calculate vectors and forecast the next sync sequence's asset trend."
    elif any(word in q for word in ["macro", "s&p", "correlation", "stock"]):
        return "We integrate the S&P 500 index via the yfinance API. By joining this with Crypto prices, the system calculates the Pearson Correlation Coefficient to mathematically determine if digital assets are acting as a macro hedge."
    elif any(word in q for word in ["hi", "hello", "hey", "who are you"]):
        return "Hello! I am the embedded Data Architecture AI for this pipeline. Ask me about the Bronze/Silver/Gold layers, the ML prediction engine, or our macroeconomic integrations."
    else:
        return "⚠️ **Out of Context:** I am strictly scoped to this pipeline's environment. I can only answer questions regarding our Data Engineering architecture, ETL steps, Machine Learning, or UI integrations."

def submit_chat():
    user_query = st.session_state.user_input
    if user_query:
        st.session_state.chat_history.append({"role": "user", "text": user_query})
        st.session_state.chat_history.append({"role": "ai", "text": generate_ai_response(user_query)})
        st.session_state.user_input = "" # Clear input

def force_chat_query(query):
    st.session_state.chat_history.append({"role": "user", "text": query})
    st.session_state.chat_history.append({"role": "ai", "text": generate_ai_response(query)})

# --- FETCH SYSTEM TIMESTAMPS ---
...
# ==========================================
# PAGE HEADER: TITLE & EXPANDABLE "ABOUT"
# ==========================================
col_title, col_about = st.columns([3, 1])
with col_title:
    st.title("🚀 Enterprise Data Platform & Predictive Intelligence")
    st.markdown("<h4 style='color: #a0b4c7;'>Cross-Asset Macro Integration, Storage Optimization & Scikit-Learn Forecasting</h4>", unsafe_allow_html=True)
with col_about:
    with st.expander("ℹ️ About this Project"):
        st.markdown("""
        **Data Engineering Medallion Pipeline**
        This end-to-end platform demonstrates advanced data architecture:
        * **Multi-Source ETL**: Fetches crypto (CoinCap) & equities (YFinance).
        * **Bronze/Silver/Gold**: Implements Databricks' Medallion architecture, handling messy JSON, compressing to PyArrow Parquet, and logging to a relational SQLite database.
        * **Machine Learning**: Features a local Scikit-Learn Regression model to predict asset vectors dynamically.
        """)
...
# ==========================================
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Manual Data Refresh"):
            with st.spinner("Executing pipeline sync..."): execute_pipeline()
            st.rerun()
        st.markdown("---")
    ...
    with tab2:
        ...
        if 'BTC' in pivot_df.columns and 'S&P500' in pivot_df.columns and len(pivot_df) > 1:
            ...
            if pd.isna(corr_value): correlation_msg = "Identical value coordinates received. Run the update loop again to generate price variance."
            elif corr_value > 0.4: correlation_msg = f"Strong Positive Correlation ({corr_value:.2f}). Crypto and Traditional Equities are climbing in tandem."
            elif corr_value < -0.4: correlation_msg = f"Strong Negative Correlation ({corr_value:.2f}). Bitcoin acts as a direct inverse market hedge."
            else: correlation_msg = f"Neutral Market Decoupling ({corr_value:.2f}). Crypto tokens are navigating independently of stock models."
        ...
        for index, row in latest_df.reset_index().iterrows():
            with cols[index]:
                if row['symbol'] == 'S&P500': st.metric(label="🇺🇸 S&P 500 Macro", value=f"{float(row['priceUsd']):,.2f} pts")
...
# ==========================================
st.write("### ")
col_empty, col_chat = st.columns([2.5, 1.5])
with col_chat:
    with st.expander("💬 Chat with Project AI Assistant", expanded=False):
        st.markdown("<div style='text-align:center; color:#a0b4c7; margin-bottom:10px;'>Ask me about this pipeline's architecture!</div>", unsafe_allow_html=True)
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("What is Medallion Arch?"): force_chat_query("Explain Medallion Architecture")
        with c2:
            if st.button("How does ML work?"): force_chat_query("How does machine learning prediction work?")
        st.markdown("</div>", unsafe_allow_html=True)
        
        # Text Input
        st.text_input("Ask a question...", key="user_input", on_change=submit_chat)
